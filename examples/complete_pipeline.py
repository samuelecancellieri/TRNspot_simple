...
def setup_directories(output_dir, figures_dir):
...
def hotspot_pipeline(adata, skip_hotspot=False):
...
Examples:
  ...
  python complete_pipeline.py --seed 123 --n-jobs 16
    ...
    parser.add_argument(
        ...
        help="Embedding name for dimensionality reduction (default: X_draw_graph_fa)",
    )
    parser.add_argument(
        "--raw-count-layer",
        type=str,
    ...
    try:
        ...
        adata = dimensionality_reduction_clustering(adata, cluster_key=args.cluster_key)
        ...
        hotspot_result = hotspot_pipeline(adata, skip_hotspot=args.skip_hotspot)

        # Generate summary
        summary = generate_summary(
            adata, celloracle_result, hotspot_result, start_time, args.output
        )
